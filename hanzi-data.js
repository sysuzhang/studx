window.LEARNING_DIMENSIONS = {
  ageGroups: [
    { value: "6-8", label: "6-8 岁（启蒙）" },
    { value: "9-12", label: "9-12 岁（基础）" },
    { value: "13-16", label: "13-16 岁（提升）" },
    { value: "16+", label: "16 岁以上（进阶）" },
  ],
  chineseLevels: [
    { value: "启蒙", label: "启蒙" },
    { value: "HSK1", label: "HSK 1" },
    { value: "HSK2", label: "HSK 2" },
    { value: "HSK3", label: "HSK 3" },
    { value: "HSK4+", label: "HSK 4+" },
  ],
};

window.HANZI_LIBRARY = [
  {
    char: "人",
    pinyin: "rén",
    meaning: "会思考、会劳动的个体，也泛指人类。",
    ages: ["6-8", "9-12"],
    levels: ["启蒙", "HSK1"],
    words: [
      { word: "人们", meaning: "许多人。" },
      { word: "大人", meaning: "成年人。" },
      { word: "人才", meaning: "有才能的人。" },
    ],
    idioms: [
      {
        name: "人山人海",
        meaning: "形容聚集的人极多。",
        story:
          "古代集市和庙会人群密集，远望如山似海，后来就用“人山人海”来形容热闹场景。",
      },
    ],
  },
  {
    char: "口",
    pinyin: "kǒu",
    meaning: "嘴巴，也可作为量词表示人口或器物的开口。",
    ages: ["6-8"],
    levels: ["启蒙", "HSK1"],
    words: [
      { word: "入口", meaning: "进入的地方。" },
      { word: "人口", meaning: "某地区的人数。" },
      { word: "口才", meaning: "说话表达能力。" },
    ],
    idioms: [],
  },
  {
    char: "山",
    pinyin: "shān",
    meaning: "地面隆起的高地。",
    ages: ["6-8", "9-12"],
    levels: ["启蒙", "HSK1"],
    words: [
      { word: "高山", meaning: "高高的山。" },
      { word: "山谷", meaning: "两山之间低凹处。" },
      { word: "山水", meaning: "山和水，也指风景。" },
    ],
    idioms: [
      {
        name: "开门见山",
        meaning: "说话写文章直截了当，切入主题。",
        story: "古人作画常先画山形定势，后来借喻说话不绕弯子，一开始就点明重点。",
      },
    ],
  },
  {
    char: "水",
    pinyin: "shuǐ",
    meaning: "无色透明的液体，是生命的重要资源。",
    ages: ["6-8", "9-12"],
    levels: ["启蒙", "HSK1", "HSK2"],
    words: [
      { word: "水果", meaning: "可食用的果实。" },
      { word: "喝水", meaning: "饮用水。" },
      { word: "水平", meaning: "能力或质量的高低程度。" },
    ],
    idioms: [
      {
        name: "滴水穿石",
        meaning: "比喻坚持不懈，终能成功。",
        story:
          "相传张乖崖见屋檐水滴长期滴在石头上留下小孔，领悟出做事贵在坚持，于是有了这个成语。",
      },
    ],
  },
  {
    char: "日",
    pinyin: "rì",
    meaning: "太阳，也指天、日子。",
    ages: ["6-8"],
    levels: ["启蒙", "HSK1"],
    words: [
      { word: "日出", meaning: "太阳升起。" },
      { word: "生日", meaning: "出生纪念日。" },
      { word: "日记", meaning: "按天记录的文字。" },
    ],
    idioms: [
      {
        name: "日积月累",
        meaning: "长时间不断积累。",
        story: "古人学习重在每天坚持，久而久之便有了“日积月累”这句劝学成语。",
      },
    ],
  },
  {
    char: "月",
    pinyin: "yuè",
    meaning: "月亮，也指月份。",
    ages: ["6-8"],
    levels: ["启蒙", "HSK1"],
    words: [
      { word: "月亮", meaning: "地球的天然卫星。" },
      { word: "月光", meaning: "月亮发出的光。" },
      { word: "本月", meaning: "这个月。" },
    ],
    idioms: [
      {
        name: "花好月圆",
        meaning: "比喻美好圆满，多用于祝福。",
        story: "传统节庆时常见花开与满月并现，象征团圆幸福，故有“花好月圆”。",
      },
    ],
  },
  {
    char: "木",
    pinyin: "mù",
    meaning: "树木，也指木材。",
    ages: ["6-8"],
    levels: ["启蒙", "HSK1"],
    words: [
      { word: "木头", meaning: "木材；也可形容人反应慢。" },
      { word: "树木", meaning: "树的总称。" },
      { word: "木工", meaning: "制作木制品的人。" },
    ],
    idioms: [],
  },
  {
    char: "火",
    pinyin: "huǒ",
    meaning: "物体燃烧时发出的光和焰。",
    ages: ["6-8", "9-12"],
    levels: ["启蒙", "HSK1"],
    words: [
      { word: "火车", meaning: "以机车牵引的列车。" },
      { word: "火花", meaning: "细小的火星。" },
      { word: "热火", meaning: "很热闹、很有劲头。" },
    ],
    idioms: [
      {
        name: "星火燎原",
        meaning: "比喻新生事物迅速发展壮大。",
        story:
          "古人观察到小火星在干燥原野可迅速蔓延，后用于比喻力量虽小却可形成大势。",
      },
    ],
  },
  {
    char: "学",
    pinyin: "xué",
    meaning: "学习，也指学问和学校。",
    ages: ["9-12", "13-16"],
    levels: ["HSK1", "HSK2"],
    words: [
      { word: "学习", meaning: "获得知识与技能。" },
      { word: "学生", meaning: "在学校读书的人。" },
      { word: "学者", meaning: "有专门学问的人。" },
    ],
    idioms: [
      {
        name: "学而不厌",
        meaning: "学习总不感到满足。",
        story:
          "出自《论语》，孔子提倡好学精神，强调学习是长期过程，不应轻易自满。",
      },
    ],
  },
  {
    char: "友",
    pinyin: "yǒu",
    meaning: "朋友，友好。",
    ages: ["9-12"],
    levels: ["HSK1", "HSK2"],
    words: [
      { word: "朋友", meaning: "彼此有交情的人。" },
      { word: "友谊", meaning: "朋友间的情谊。" },
      { word: "友好", meaning: "和睦亲近。" },
    ],
    idioms: [
      {
        name: "良师益友",
        meaning: "能给予帮助和启发的老师与朋友。",
        story: "古人认为学习离不开师友相助，能促人成长的人，就可称“良师益友”。",
      },
    ],
  },
  {
    char: "家",
    pinyin: "jiā",
    meaning: "家庭，也指某一方面有专长的人。",
    ages: ["9-12"],
    levels: ["HSK1", "HSK2"],
    words: [
      { word: "家庭", meaning: "共同生活的亲属成员群体。" },
      { word: "国家", meaning: "由领土、人民、政权组成的政治共同体。" },
      { word: "专家", meaning: "在某领域有专门知识的人。" },
    ],
    idioms: [
      {
        name: "家喻户晓",
        meaning: "每家每户都知道，形容非常有名。",
        story: "古代信息传播慢，能“家喻户晓”的事情说明影响极广，因此该成语常用于赞誉。",
      },
    ],
  },
  {
    char: "书",
    pinyin: "shū",
    meaning: "书籍；写字、记录。",
    ages: ["9-12", "13-16"],
    levels: ["HSK1", "HSK2"],
    words: [
      { word: "读书", meaning: "阅读书籍，也指学习。" },
      { word: "书法", meaning: "写字的艺术。" },
      { word: "书包", meaning: "装书和文具的包。" },
    ],
    idioms: [
      {
        name: "博览群书",
        meaning: "广泛阅读各种书籍。",
        story:
          "古代文人重视阅读积累，涉猎越广，见识越深，因此“博览群书”成为治学理想。",
      },
    ],
  },
  {
    char: "春",
    pinyin: "chūn",
    meaning: "一年四季中的第一季，象征生机。",
    ages: ["9-12", "13-16"],
    levels: ["HSK1", "HSK2"],
    words: [
      { word: "春天", meaning: "春季。" },
      { word: "春风", meaning: "春季和暖的风。" },
      { word: "春节", meaning: "农历新年。" },
    ],
    idioms: [
      {
        name: "春暖花开",
        meaning: "春天气候温暖，百花盛开，也比喻美好时机。",
        story: "人们经历寒冬后迎来春日繁花，感受希望与新生，于是形成这一成语。",
      },
    ],
  },
  {
    char: "风",
    pinyin: "fēng",
    meaning: "空气流动形成的自然现象，也可指风尚。",
    ages: ["9-12", "13-16"],
    levels: ["HSK1", "HSK2", "HSK3"],
    words: [
      { word: "风景", meaning: "可供观赏的景色。" },
      { word: "风格", meaning: "作风或艺术特色。" },
      { word: "台风", meaning: "热带气旋的一种。" },
    ],
    idioms: [
      {
        name: "风和日丽",
        meaning: "微风和煦，阳光明媚。",
        story:
          "古人常用天气描绘好日子，“风和日丽”既写景也写心情，后来成为常用祝福语。",
      },
    ],
  },
  {
    char: "明",
    pinyin: "míng",
    meaning: "亮；清楚；懂得。",
    ages: ["9-12", "13-16"],
    levels: ["HSK2", "HSK3"],
    words: [
      { word: "明白", meaning: "清楚、懂得。" },
      { word: "聪明", meaning: "理解和反应能力强。" },
      { word: "明天", meaning: "今天之后的一天。" },
    ],
    idioms: [
      {
        name: "光明磊落",
        meaning: "胸怀坦荡，正大光明。",
        story:
          "古代士人重气节，行事坦荡不隐瞒，被赞为“光明磊落”，后成为人格评价词。",
      },
    ],
  },
  {
    char: "德",
    pinyin: "dé",
    meaning: "品德，道德品质。",
    ages: ["13-16", "16+"],
    levels: ["HSK3", "HSK4+"],
    words: [
      { word: "品德", meaning: "道德品质。" },
      { word: "德育", meaning: "培养道德品质的教育。" },
      { word: "美德", meaning: "优秀的道德品质。" },
    ],
    idioms: [
      {
        name: "厚德载物",
        meaning: "指人应当以深厚德行包容万物。",
        story:
          "语出《周易》“地势坤，君子以厚德载物”，强调做人要胸怀宽广、品德深厚。",
      },
    ],
  },
  {
    char: "信",
    pinyin: "xìn",
    meaning: "诚实不欺；信用；消息或书信。",
    ages: ["13-16", "16+"],
    levels: ["HSK3", "HSK4+"],
    words: [
      { word: "信用", meaning: "按约履行承诺的可靠程度。" },
      { word: "相信", meaning: "认为正确或可靠。" },
      { word: "书信", meaning: "信件。" },
    ],
    idioms: [
      {
        name: "言而有信",
        meaning: "说话守信用。",
        story:
          "古人把诚信视为立身之本，承诺后必须兑现，“言而有信”由此成为做人准则。",
      },
    ],
  },
  {
    char: "志",
    pinyin: "zhì",
    meaning: "志向，意志，心意。",
    ages: ["13-16", "16+"],
    levels: ["HSK3", "HSK4+"],
    words: [
      { word: "志向", meaning: "人生目标和理想。" },
      { word: "意志", meaning: "坚持达到目标的心理力量。" },
      { word: "同志", meaning: "志同道合的人。" },
    ],
    idioms: [
      {
        name: "志在四方",
        meaning: "形容有远大志向，不满足于小天地。",
        story:
          "古时青年多立志出游求学建功，胸怀天下，因此有“志在四方”的说法。",
      },
    ],
  },
  {
    char: "勤",
    pinyin: "qín",
    meaning: "勤劳，勤奋，不懒惰。",
    ages: ["13-16", "16+"],
    levels: ["HSK3", "HSK4+"],
    words: [
      { word: "勤奋", meaning: "努力不懈。" },
      { word: "勤劳", meaning: "爱劳动，不怕辛苦。" },
      { word: "值勤", meaning: "值班执行任务。" },
    ],
    idioms: [
      {
        name: "勤能补拙",
        meaning: "勤奋能够弥补天赋或能力的不足。",
        story: "许多古代人物以勤学改变命运，后人总结为“勤能补拙”来鼓励学习者。",
      },
    ],
  },
  {
    char: "诚",
    pinyin: "chéng",
    meaning: "真诚，不虚假。",
    ages: ["13-16", "16+"],
    levels: ["HSK3", "HSK4+"],
    words: [
      { word: "诚实", meaning: "言行真实，不欺骗。" },
      { word: "真诚", meaning: "真实诚恳。" },
      { word: "诚意", meaning: "真心实意。" },
    ],
    idioms: [
      {
        name: "开诚布公",
        meaning: "坦率真诚地表达意见。",
        story:
          "古人议政强调坦诚交流，“开诚布公”最初多用于政治讨论，后泛指真诚沟通。",
      },
    ],
  },
  {
    char: "和",
    pinyin: "hé",
    meaning: "和谐；温和；不冲突。",
    ages: ["9-12", "13-16", "16+"],
    levels: ["HSK2", "HSK3", "HSK4+"],
    words: [
      { word: "和平", meaning: "没有战争和冲突。" },
      { word: "和气", meaning: "态度温和可亲。" },
      { word: "和谐", meaning: "协调而平衡。" },
    ],
    idioms: [
      {
        name: "和衷共济",
        meaning: "同心协力，克服困难。",
        story:
          "古人强调集体合作，“和衷共济”本意是同心渡河，后来比喻团结协作。",
      },
    ],
  },
  {
    char: "智",
    pinyin: "zhì",
    meaning: "智慧，聪明才智。",
    ages: ["13-16", "16+"],
    levels: ["HSK3", "HSK4+"],
    words: [
      { word: "智慧", meaning: "辨析判断和创造能力。" },
      { word: "智力", meaning: "认识和思考能力。" },
      { word: "机智", meaning: "灵活聪明。" },
    ],
    idioms: [
      {
        name: "大智若愚",
        meaning: "有大智慧的人表面上反而显得朴实。",
        story:
          "古人认为真正的智者不炫耀聪明，看似平常却洞察深刻，因此有“大智若愚”。",
      },
    ],
  },
  {
    char: "礼",
    pinyin: "lǐ",
    meaning: "礼节、礼貌，也指礼仪制度。",
    ages: ["13-16", "16+"],
    levels: ["HSK3", "HSK4+"],
    words: [
      { word: "礼貌", meaning: "言行举止有礼节。" },
      { word: "礼仪", meaning: "社交和典礼的规范。" },
      { word: "礼物", meaning: "赠送他人的物品。" },
    ],
    idioms: [
      {
        name: "礼尚往来",
        meaning: "礼节上要有来有往。",
        story:
          "古代交往讲究“往而不来，非礼也”，强调互相尊重，因此形成“礼尚往来”。",
      },
    ],
  },
  {
    char: "思",
    pinyin: "sī",
    meaning: "想，思考。",
    ages: ["9-12", "13-16"],
    levels: ["HSK2", "HSK3"],
    words: [
      { word: "思想", meaning: "思维活动和观念。" },
      { word: "思考", meaning: "用头脑分析问题。" },
      { word: "相思", meaning: "彼此思念。" },
    ],
    idioms: [
      {
        name: "深思熟虑",
        meaning: "深入反复地考虑。",
        story:
          "古代做重大决定前常多次商议，“深思熟虑”因此成为做事稳重的代名词。",
      },
    ],
  },
  {
    char: "行",
    pinyin: "xíng",
    meaning: "走；做；可以。",
    ages: ["9-12", "13-16", "16+"],
    levels: ["HSK2", "HSK3", "HSK4+"],
    words: [
      { word: "行动", meaning: "做事的行为。" },
      { word: "可行", meaning: "可以实施。" },
      { word: "旅行", meaning: "到外地游历。" },
    ],
    idioms: [
      {
        name: "身体力行",
        meaning: "亲身去做，努力实践。",
        story:
          "很多先贤主张“知行合一”，不仅懂道理，更要亲自实践，故有“身体力行”。",
      },
    ],
  },
  {
    char: "远",
    pinyin: "yuǎn",
    meaning: "距离长；时间久；关系疏。",
    ages: ["9-12", "13-16", "16+"],
    levels: ["HSK2", "HSK3", "HSK4+"],
    words: [
      { word: "远方", meaning: "很远的地方。" },
      { word: "长远", meaning: "时间跨度较久。" },
      { word: "远见", meaning: "长远的眼光。" },
    ],
    idioms: [
      {
        name: "高瞻远瞩",
        meaning: "站得高、看得远，比喻眼光长远。",
        story:
          "古代将领登高观察地势来制定策略，后人借此形容做事有全局与长远视角。",
      },
    ],
  },
  {
    char: "新",
    pinyin: "xīn",
    meaning: "刚出现或刚经历过；不旧。",
    ages: ["9-12", "13-16"],
    levels: ["HSK1", "HSK2", "HSK3"],
    words: [
      { word: "新年", meaning: "新的一年。" },
      { word: "新鲜", meaning: "刚生产或刚出现，不陈旧。" },
      { word: "创新", meaning: "提出新的方法或成果。" },
    ],
    idioms: [
      {
        name: "推陈出新",
        meaning: "去掉旧的，创造新的。",
        story:
          "历代工艺和学术都在继承中发展，“推陈出新”体现了传统与创新并重的思想。",
      },
    ],
  },
];

window.HANZI_PICTOGRAPH_MAP = {
  人: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/c/cd/%E4%BA%BA-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E4%BA%BA-oracle.svg",
    note: "早期字形像侧立之人，体现“人”的象形来源。",
  },
  口: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/9/99/%E5%8F%A3-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E5%8F%A3-oracle.svg",
    note: "字形接近口部轮廓，属于典型象形字。",
  },
  山: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/6/6b/%E5%B1%B1-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E5%B1%B1-oracle.svg",
    note: "峰峦起伏的形态非常直观，便于理解“山”字本义。",
  },
  水: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/7/75/%E6%B0%B4-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E6%B0%B4-oracle.svg",
    note: "线条表现流水分支，体现“水流”形象。",
  },
  日: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/4/44/%E6%97%A5-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E6%97%A5-oracle.svg",
    note: "圆中有点的结构常用来表示太阳。",
  },
  月: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/9/94/%E6%9C%88-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E6%9C%88-oracle.svg",
    note: "弯月轮廓演变为“月”字，形义关联明显。",
  },
  木: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/e/e9/%E6%9C%A8-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E6%9C%A8-oracle.svg",
    note: "树干与枝杈的组合形成“木”字雏形。",
  },
  火: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/4/40/%E7%81%AB-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E7%81%AB-oracle.svg",
    note: "火焰上扬的姿态在古文字中保留明显。",
  },
  学: {
    script: "甲骨文（繁体：學）",
    image: "https://upload.wikimedia.org/wikipedia/commons/2/24/%E5%AD%B8-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E5%AD%B8-oracle.svg",
    note: "采用“學”古文字字形，展示学习含义的早期表达。",
  },
  友: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/f/f8/%E5%8F%8B-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E5%8F%8B-oracle.svg",
    note: "古字形常体现并肩协助之意，贴合“友”字语义。",
  },
  家: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/1/12/%E5%AE%B6-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E5%AE%B6-oracle.svg",
    note: "古文字中可见“宀”与“豕”要素，反映“家”字来源。",
  },
  书: {
    script: "篆书（繁体：書）",
    image: "https://upload.wikimedia.org/wikipedia/commons/9/9e/%E6%9B%B8-seal.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E6%9B%B8-seal.svg",
    note: "采用“書”篆书字形，帮助理解“书写”意义演变。",
  },
  春: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/c/cd/%E6%98%A5-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E6%98%A5-oracle.svg",
    note: "字形体现季节更替中的生长意象。",
  },
  风: {
    script: "篆书（繁体：風）",
    image: "https://upload.wikimedia.org/wikipedia/commons/3/3f/%E9%A2%A8-seal.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E9%A2%A8-seal.svg",
    note: "采用“風”篆书字形，古今写法存在繁简差异。",
  },
  明: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/4/4a/%E6%98%8E-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E6%98%8E-oracle.svg",
    note: "“日月相照”是“明”字常见字源解释。",
  },
  德: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/3/31/%E5%BE%B7-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E5%BE%B7-oracle.svg",
    note: "古文字结构较复杂，适合结合释义理解字义演化。",
  },
  信: {
    script: "字形演示图",
    image: "https://upload.wikimedia.org/wikipedia/commons/a/a7/%E4%BF%A1-order.gif",
    source: "https://commons.wikimedia.org/wiki/File:%E4%BF%A1-order.gif",
    note: "当前以字形演示图补充展示，帮助记忆“信”字结构。",
  },
  志: {
    script: "字形参考图",
    image: "https://upload.wikimedia.org/wikipedia/commons/1/11/%E5%BF%97-bw.png",
    source: "https://commons.wikimedia.org/wiki/File:%E5%BF%97-bw.png",
    note: "当前以字形参考图展示，后续可继续补充古文字版本。",
  },
  勤: {
    script: "字形结构图",
    image:
      "https://upload.wikimedia.org/wikipedia/commons/f/fb/%E5%8B%A4_-_U%2B052E4-_KanjiVG_stroke_order.svg",
    source:
      "https://commons.wikimedia.org/wiki/File:%E5%8B%A4_-_U%2B052E4-_KanjiVG_stroke_order.svg",
    note: "当前公开古文字资源较少，先提供结构图用于形体识记。",
  },
  诚: {
    script: "篆书（繁体：誠）",
    image: "https://upload.wikimedia.org/wikipedia/commons/2/21/%E8%AA%A0-seal.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E8%AA%A0-seal.svg",
    note: "采用“誠”篆书字形，体现繁简字形关联。",
  },
  和: {
    script: "简牍字形",
    image: "https://upload.wikimedia.org/wikipedia/commons/5/5d/%E5%92%8C-slip.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E5%92%8C-slip.svg",
    note: "简牍写法可用于观察“和”字在历史中的形体变化。",
  },
  智: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/3/31/%E6%99%BA-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E6%99%BA-oracle.svg",
    note: "古字形强调构件组合，有助于理解“智”的义素。",
  },
  礼: {
    script: "甲骨文（繁体：禮）",
    image: "https://upload.wikimedia.org/wikipedia/commons/b/b9/%E8%B1%8A-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E8%B1%8A-oracle.svg",
    note: "礼字古形常与“禮/豐”相关，体现礼仪文化来源。",
  },
  思: {
    script: "大篆",
    image: "https://upload.wikimedia.org/wikipedia/commons/3/32/%E6%80%9D-bigseal.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E6%80%9D-bigseal.svg",
    note: "大篆字形可用于对比现代“思”字结构。",
  },
  行: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/8/8f/%E8%A1%8C-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E8%A1%8C-oracle.svg",
    note: "古字形像道路交汇，贴合“行走/行动”含义。",
  },
  远: {
    script: "甲骨文（繁体：遠）",
    image: "https://upload.wikimedia.org/wikipedia/commons/d/d1/%E9%81%A0-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E9%81%A0-oracle.svg",
    note: "采用“遠”古形，帮助理解“辶”相关语义。",
  },
  新: {
    script: "甲骨文",
    image: "https://upload.wikimedia.org/wikipedia/commons/d/dd/%E6%96%B0-oracle.svg",
    source: "https://commons.wikimedia.org/wiki/File:%E6%96%B0-oracle.svg",
    note: "古文字展示“新”字构件来源与组合方式。",
  },
};

window.HANZI_LIBRARY = window.HANZI_LIBRARY.map((item) => ({
  ...item,
  pictograph: window.HANZI_PICTOGRAPH_MAP[item.char] || null,
}));
